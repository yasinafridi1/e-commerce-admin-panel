...
const Index = ({ sideBarOpen, setSideBar }: { sideBarOpen: boolean, setSideBar: React.Dispatch<React.SetStateAction<boolean>> }) => {
    const location = useLocation();
    const { pathname } = location;
    const trigger = useRef<HTMLButtonElement | null>(null);
    const sidebar = useRef<HTMLDivElement | null>(null);

    const storedSidebarExpanded = getLocalStorageValue(CONSTANTS.sidebarExpanded);
    ...
    useEffect(() => {
        storeInLocalStorage(CONSTANTS.sidebarExpanded, sidebarExpanded.toString());
        if (sidebarExpanded) {
    ...
    return (
        ...
        >
            ...
            <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
                <nav className="mt-3 py-4 px-2 lg:mt-3 lg:px-6">
                    <div>
